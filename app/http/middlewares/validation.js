...
const validate = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return res.render("component/jobApplication/insertForm", {
        combos,
        data,
    ...
    board_name.forEach((element, index) => {
      if (
        !board_name[index].trim() ||
        !passing_year[index].trim() ||
        !percentage[index].trim()
      ) {
    ...
    board_name.forEach((element, index) => {
      if (
        passing_year[index].trim().length < 4 ||
        passing_year[index].trim().length > 4
      ) {
    ...
    if (
      bachelor_passing_year.trim().length < 4 ||
      bachelor_passing_year.trim().length > 4
    ) {
      req.flash(
        "error_education",
        "Bachelor Passing Year  have to be 4 digit "
      );
      return res.render("component/jobApplication/insertForm", {
        combos,
        data,
      ...
      req.flash(
        "error_education",
        "Bachelor Passing Year have to be only digit"
      );
      return res.render("component/jobApplication/insertForm", {
        combos,
        data,
    ...
    if (
      !(
        master_cgpa.trim() == "" &&
        master_course_name.trim() == "" &&
        master_passing_year.trim() == "" &&
        master_university.trim() == ""
      )
    ) {
      if (
        master_cgpa.trim() == "" ||
    ...
    for (let i = 0; i < company_name.length; i++) {
      if (
        !(
          company_name[i].trim() == "" &&
          work_experience_designation[i].trim() == "" &&
          from[i].trim() == "" &&
          to[i].trim() == ""
        )
      ) {
        if (
          ...
          req.flash(
            "error_experience",
            "Required All Detail of work Experience if you have"
          );
          return res.render("component/jobApplication/insertForm", {
            combos,
            data,
          ...
          if (to[i].length <= 8) {
            ...
            if (isNaN(new Date(to[i].trim()))) {
              req.flash(
                "error_experience",
                "Enter To date value in valid form"
              );
              return res.render("component/jobApplication/insertForm", {
                combos,
                data,
    ...
    for (let i = 0; i < language.length; i++) {
      if (
        !data[`${data.language[i]}_read`] &&
        !data[`${data.language[i]}_write`] &&
        !data[`${data.language[i]}_speak`]
      ) {
        req.flash(
          "error_language",
          "Required Atleast one language opration for each selected language"
        );
        return res.render("component/jobApplication/insertForm", {
          combos,
          data,
    ...
    if (technology) {
      for (let i = 0; i < technology.length; i++) {
        if (
          !data[`${data.technology[i]}_level`] &&
          !data[
            `${data.technology[i]}_level` &&
              !data[`${data.technology[i]}_level`]
          ]
        ) {
          req.flash(
            "error_language",
            "Required Atleast one technology level for each selected technology"
          );
          return res.render("component/jobApplication/insertForm", {
            combos,
            data,
    ...
    for (let i = 0; i < referance_name.length; i++) {
      if (
        !(
          referance_name[i].trim() == "" &&
          referance_number[i].trim() == "" &&
          referance_relation[i].trim() == ""
        )
      ) {
        if (
          referance_name[i].trim() == "" ||
          ...
          if (
            referance_number[i].trim().length < 10 ||
            referance_number[i].trim().length > 10
          ) {
    ...
    if (
      !preferance_curr_ctc.trim() ||
      !preferance_expec_ctc.trim() ||
      !preferance_notice.trim() ||
      !prefered_location.trim() ||
      !department.trim()
    ) {
