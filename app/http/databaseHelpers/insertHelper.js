       ...
       async basic_insert(req,res){
        ...
        async board_insert(req,res,employee_id){
                ...
                board.forEach(async(item,index)=>{
                    let details = {
                       ...
                       conn.query('insert into board_detail SET ?',details).then((res)=>{
                        console.log(res);
                       }).catch((err)=>{
                        console.log(err)
                       })
        ...
        async degree_insert(req,res,employee_id){
            try {
               ...
               conn.query('insert into degree_detail SET ?',bechlor_details).then((res)=>{
                console.log(res);
               }).catch((err)=>{
                console.log(err)
               })
                ...
                if (data.master_cgpa && data.master_course_name && data.master_passing_year && data.master_university) {
                    ...
                    conn.query('insert into degree_detail SET ?',master_details).then((res)=>{
                        console.log(res);
                       }).catch((err)=>{
                        console.log(err)
                       })
        ...
        async work_experience(req,res,employee_id){
            try {
                ...
                for (let i = 0; i < data.company_name.length; i++) {
                    ...
                    conn.query('insert into workexperience_detail SET ?',workExperience_details).then((res)=>{
                        console.log(res);
                       }).catch((err)=>{
                        console.log(err)
                       })
        ...
        async reference(req,res,employee_id){
            ...
            for (let i = 0; i < data.referance_name.length; i++) {
                if (data.referance_name[i] != ''  && data.referance_number[i] != '' && data.referance_relation != '') {
                 ...
                 conn.query('insert into refered_contact_detail SET ?',reference_details).then((res)=>{
                    console.log(res);
                   }).catch((err)=>{
                    console.log(err)
                   })
