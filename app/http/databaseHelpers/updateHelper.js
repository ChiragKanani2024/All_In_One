       ...
       async basic_update(req,res,employee_id){
        ...
        async board_update(req,res,employee_id){
                ...
                board.forEach(async(item,index)=>{
                    let details = {
                       ...
                       conn.query(`update board_detail SET ? where employee_id = ${employee_id} and board = '${item}'`,details).then((res)=>{
                       }).catch((err)=>{
                        console.log(err)
                       })
        ...
        async degree_update(req,res,employee_id){
            try {
               ...
               conn.query(`update degree_detail SET ? where employee_id = ${employee_id} and degree='Bechelor'`,bechlor_details).then((res)=>{
               }).catch((err)=>{
                console.log(err)
               })
                
                if (data.master_cgpa && data.master_course_name && data.master_passing_year && data.master_university) {
                    let master_details = {
                    ...
                    conn.query(`update degree_detail SET ? where employee_id = ${employee_id} and degree='Master'`,master_details).then((res)=>{
                       }).catch((err)=>{
                        console.log(err)
                       })
                }
        ...
        async work_experience_update(req,res,employee_id){
            try {
                ...
                for (let i = 0; i < data.company_name.length; i++) {
                    if (data.company_name[i] != '' && data.work_experience_designation[i] != '' && data.from[i] != '' && data.to[i] != '' ) {
                    ...
                    conn.query(`update workexperience_detail SET ? where id=${data.work_id[i]}`,workExperience_details).then((res)=>{
                       }).catch((err)=>{
                        console.log(err)
                       })
                    }     
        ...
        async reference_update(req,res,employee_id){
            ...
            for (let i = 0; i < data.referance_name.length; i++) {
                if (data.referance_name[i] != ''  && data.referance_number[i] != '' && data.referance_relation != '') {
                 ...
                 conn.query(`update refered_contact_detail SET ? where id= ${data.ref_id[i]}`,reference_details).then((res)=>{
                   }).catch((err)=>{
                    console.log(err)
                   })
                }
            ...
            try {
                ...
                for (let i = 0; i < data.language.length; i++) {
                  ...
                  if (languages.includes(data.language[i])) {
                    conn.query(`update language_detail SET ? where employee_id = ${employee_id} and language = '${data.language[i]}'`,language_details).then(async(res)=>{
                    }).catch((err)=>{
                     console.log(err)
                    })
                  }else{
                    conn.query('insert into language_detail SET ?',language_details).then((res)=>{
                       }).catch((err)=>{
                        console.log(err)
                       })
                  }
                ...
                if(languageDetail.length != data.language.length){
                    ...
                    conn.query(`delete from language_detail where employee_id = ${employee_id} and language = '${item.language}'`).then(res=>{}).catch(err=>{console.log(err)})
                            }
                        })
                    }
                }
        ...
        async technology_update(req,res,employee_id){
            try {
                let technologyDetail = await getDataHelper().technology_detail(employee_id);
                let data = req.body;
                let technologys = [];
                ...
                for (let i = 0; i < data.technology.length; i++) {
                    ...
                    if (technologys.includes(data.technology[i])) {
                        conn.query(`update technology_detail SET ? where employee_id = ${employee_id} and technology = '${data.technology[i]}'`,technology_details).then((res)=>{
                        }).catch((err)=>{
                         console.log(err)
                        })
                    }else{
                        conn.query('insert into technology_detail SET ?',technology_details).then((res)=>{
                           }).catch((err)=>{
                            console.log(err)
                           })
                    }
                ...
                if(technologyDetail.length != data.technology.length){
                    ...
                    conn.query(`delete from technology_detail where employee_id = ${employee_id} and technology = '${item.technology}'`).then(res=>{}).catch(err=>{console.log(err)})
                            }
                        })
                    }
                }
