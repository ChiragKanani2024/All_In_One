    ...
    return {
        async getform(req,res){
            try {
                ...
                let technology_detail = await  getDataHelper().technology_detail(employee_id)
                let language_detail = await getDataHelper().language_detail(employee_id)
                let refered_detail = await  getDataHelper().refered_detail(employee_id)
                let work_detail = await getDataHelper().work_experience_detail(employee_id)
                ...
                let data ={
                    ...
                    board_name: [ board_detail[0].nameofboard,board_detail[1].nameofboard ],
                    passing_year: [ board_detail[0].passing_year,board_detail[1].passing_year],
                    ...
                    master_university: degree_detail[1]?degree_detail[1].university:'',
                    master_passing_year: degree_detail[1]?degree_detail[1].passingyear:'',
                ...
                let genderRadio =await comboGenerate('gender_radio',basic_detail.gender);
                let relationShipCombo = await comboGenerate('relationship_combo',basic_detail.status);
                let locationCombo = await comboGenerate('prefered_location_combo',basic_detail.prefered_location);
                let departmentCombo = await comboGenerate('department_combo',basic_detail.department)
                let statesCombo = await comboGenerate('state_combo',basic_detail.state);
        ...
        async updateData(req,res){
            ...
            try {
                ...
                try {
                    let result = await updateHelper().degree_update(req,res,employee_id);
                } catch (error) {
                    res.send(error.sqlMessage)
                }
                try {
                    let result = await updateHelper().work_experience_update(req,res,employee_id);
                } catch (error) {
                    res.send(error.sqlMessage)
                }
                try {
                    let result = await updateHelper().reference_update(req,res,employee_id);
                } catch (error) {
                    res.send(error.sqlMessage)
                }
                try {
                    let result = await updateHelper().language_update(req,res,employee_id);
                } catch (error) {
                    res.send(error.sqlMessage)
                }
                try {
                    let result = await updateHelper().technology_update(req,res,employee_id);
                } catch (error) {
                    res.send(error.sqlMessage)
                }
            if (req.get("xml")) {
