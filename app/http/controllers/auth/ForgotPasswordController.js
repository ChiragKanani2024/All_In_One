      ...
      try {
        ...
        if (result[0].length >= 1) {
          if (activation_key == result[0][0].active_pin) {
            ...
            if (diff < 3) {
              res.render("auth/forgotpassinputs", {
        ...
        if (result[0].length >= 1) {
          if (result[0][0].status === "Active") {
            let update = await conn.query(
              `update Users SET pass_updated_at = CURRENT_TIMESTAMP where email = '${req.body.email}'`
            );
      ...
      try {
        if (!req.body.password.trim() || !req.body.conform_password.trim()) {
          ...
          if (req.body.password !== req.body.conform_password) {
            return res.send({
              success: false,
              alert: "Both Password Not Matched Try Again",
            });
          } else {
            let result = await conn.query(
              `select * from Users where email = '${req.body.email}'`
