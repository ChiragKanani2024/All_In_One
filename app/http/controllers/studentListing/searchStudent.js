...
const searchById = async (req, res) => {
  try {
    ...
    let result = await conn.query(
      `${req.session.filterByIdquery} order by ${
        req.session.orderby ? req.session.orderby : "id"
    ...
    res.render("component/studentListing/index", {
      ...
      route: "student_listing/searchById",
...
const searchByFilter = async (req, res) => {
  try {
    ...
    if (req.method == "POST") {
      ...
      firstnameFilter =
        req.body.firstname !== ""
          ? `first_name like '${req.body.firstname}%'`
          : `first_name like '%'`;
      lastnameFilter =
        req.body.lastname !== ""
          ? `last_name like '${req.body.lastname}%'`
          : `last_name like '%'`;
      cityFilter =
        req.body.city !== ""
    ...
    let result = await conn.query(
      `${req.session.filterquery} order by ${
        req.session.orderby ? req.session.orderby : "id"
      }  limit ${limit} offset ${offset} `
    );
    ...
    res.render("component/studentListing/index", {
      ...
      limit: limit,
      orderby: req.session.orderby,
      route: "student_listing/searchByFilter",
      layout: "layouts/student_listing.ejs",
    });
  } catch (error) {
    console.log(error);
