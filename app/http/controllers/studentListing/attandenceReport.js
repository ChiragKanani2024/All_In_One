...
const getattandence = async (offset, limit, filterbyMonth, req) => {
  try {
    ...
    let result =
      await conn.query(`select s.id,concat(s.first_name," ",s.last_name) as student_name,count(s.id) as presentDay from Student_Master as s
            LEFT JOIN attandence_Master as a on s.id = a.student_id  where DATE_FORMAT(a.date, '%Y-%m')  = ${
              filterbyMonth
                ? `'${filterbyMonth}'`
                : `'${year}-${`${month}`.padStart(2, 0)}'`
            } and a.attandence = 1 group by s.id limit ${limit} offset ${offset}`);
