...
const getStudentResult = async (id) => {
  try {
    ...
    let result =
      await conn.query(`select e.student_id,concat(s.first_name," ",s.last_name) as student_name,sub.name,
            sum(case 
    ...
    let attandence = await conn.query(
      `select count(student_id) as present_day,DATE_FORMAT(date, '%Y-%m') as month , day( LAST_DAY(date)) as monthday from  attandence_Master where attandence = 1 and student_id = ${id} group by DATE_FORMAT(date, '%Y-%m') , monthday`
    );
