...
const getStudentData = async(req,res)=>{
    try {
        ...
        let colname={"_":'first_name',"^":'last_name', "}":'age',":":'city'}
        ...
        for (const key in filter) {
           if( where[key]){
            if(where[key].length >1){
                ...
                filter[key] += ")"
            }else{filter[key] = `${colname[key]} like '%${where[key][0]}%'`}
           }
        }
        ...
        let DataCount = await conn.query(`select count(*) as count from (${req.session.query?req.session.query:'select * from Student_Master'}) as r`)
        ...
        let result = await conn.query(`${req.session.query?req.session.query:'select * from Student_Master'} limit ${limit} offset ${offset} `)

     
        res.render("component/delemeter_search/delemeter_search",{users:result[0],firstpage:1,lastpage:lastPage,currentpage:page,TotalDataCount:DataCount[0][0].count,limit:limit,filter:req.body.string,layout: 'layouts/delemeter_layout.ejs'})
    } catch (error) {
        console.log(error)
    }
}
