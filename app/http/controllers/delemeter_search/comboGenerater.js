...
const generate = async (unique_key, value) => {
  try {
    ...
    if (select.control_type == "radio" || select.control_type == "checkbox") {
      ...
      data.forEach((element) => {
        string += `<input type="${select.control_type}" ${
          element.value == value ? `checked` : ``
        } value=${element.value} name=${select.name}>${element.name} `;
      });
      return string;
    }
  } catch (error) {
...
const comboGenerate = async (req, res) => {
  try {
    ...
    res.render("component/delemeter_search/combo", {
      combo,
      layout: "plainLayout.ejs",
    });
  } catch (error) {
    console.log(error);
  }
